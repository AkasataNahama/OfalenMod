...
public class TileEntityPlacer extends TileEntityWorldEditorBase implements IInventory {
	...
	protected boolean work(int x, int y, int z) {
		...
		try {
			...
			OfalenLog.error("Failed to place block. (name=" + block.getLocalizedName() + ", id=" + Block.getIdFromBlock(block) + ", meta=" + meta + ")", "TileEntityPlacer");
			OfalenLog.error("This error was anticipated. Probably Placer failed to place the block.", "TileEntityPlacer");
			e.printStackTrace();
			return false;
		}
