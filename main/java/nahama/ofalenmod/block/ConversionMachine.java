...
public class ConversionMachine extends BlockContainer {

	protected final Random random = new Random();
	private IIcon[] iicon = new IIcon[4];
	private static boolean isBurning;

	public ConversionMachine() {
		super(Material.rock);
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		((TileEntityConversionMachine) world.getTileEntity(x, y, z)).setGrade(getGrade(world, x, y, z));
		// GUIを開く
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack itemStack) {
		...
		if (itemStack.hasDisplayName()) {
			((TileEntityConversionMachine) world.getTileEntity(x, y, z)).setCustomName(itemStack.getDisplayName());
		}
	}
	...
	public static void updateBlockState(boolean burning, World world, int x, int y, int z) {
		...
		if (tileentity != null) {
			...
			((TileEntityConversionMachine) tileentity).setGrade(getGrade(world, x, y, z));
		}
	}
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
		if (!isBurning) {
			TileEntityConversionMachine tileentity = (TileEntityConversionMachine) world.getTileEntity(x, y, z);
